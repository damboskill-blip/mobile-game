...
export function update(world, dt) {
  ...
  // Leather counter money piles
  for (let i = world.leatherCounter.moneyPiles.length - 1; i >= 0; i--) {
    const pile = world.leatherCounter.moneyPiles[i];
    const d = Math.hypot(pile.pos.x - p.pos.x, pile.pos.z - p.pos.z);
    if (d <= r) {
      world.money.pocket += pile.amount;
      world.leatherCounter.moneyPiles.splice(i, 1);
    }
  }
}
